...
1. 完善了《损失函数》中的 “Dice Loss：区域重叠损失”、“Focal Loss：难样本聚焦损失”、“Lovász Loss：IoU 近似优化损失” 章节，补充数学原理、具体计算、PyTorch 实现、适用任务与扩展内容。
2. 更新了《评价指标》中的 “通用评价指标” 章节，从混淆矩阵出发，整理 Accuracy、Precision、Recall 与 F1 的数学公式、直觉解释和 PyTorch / NumPy 实现。
3. 创建并更新了《评价指标》中的 “目标检测评价指标” 章节，整理边界框 IoU、预测框匹配、PR 曲线、AP 与 mAP 的完整计算过程。
4. 创建并更新了《跨时代算法》中的 “目标检测算法：找出目标及其位置” 章节，从稳定算法骨架、预测位置、边界框起点、匹配与去重等设计问题出发，梳理 R-CNN、YOLO / SSD、DETR 与端到端检测路线。
5. 创建并更新了《跨时代算法》中的 “实例分割算法：为每个目标生成独立 Mask” 章节，整理 Mask R-CNN、YOLACT / SOLO、MaskFormer / Mask2Former 与 SAM 1/2/3 的演进，并区分固定类别实例分割、提示分割和开放词汇实例分割。

### 2026 年 06 月

1. 创建并更新了 《归一化方法》中的 “批归一化”、“层归一化”、“实例归一化”、“组归一化”、“均方根归一化” 章节。
2. 完结了《网络结构》中的 “CNN 家族总结：演进脉络与并行分支” 章节，补充了 MobileNetV3/V4、NAS、SE、CBAM、ResNeXt 与 CNN 演进全景图。
3. 创建并更新了《跨时代算法》中的 “图像分类算法：流程稳定” 章节，从分类任务定义、训练闭环、固定组件、网络结构演进四个角度整理图像分类。
4. 创建并更新了《跨时代算法》中的 “语义分割算法：像素级分类” 章节，将语义分割抽象为像素级分类，并梳理 FCN、U-Net、DeepLab、SegFormer、MaskFormer 等结构路线。
5. 创建了《评价指标》模块，更新了 “分割任务评价指标” 章节，整理 mIoU 与 Dice 的数学公式、直觉解释、PyTorch / NumPy 使用方式和手写实现。
6. 在《损失函数》中创建了 “Dice Loss”、“Focal Loss”、“Lovasz Loss” 三个占位章节，用于后续补充分割任务相关损失函数。
7. 更新了深度学习模块的导航栏与侧边栏，补充《归一化方法》和《评价指标》入口，并将《跨时代算法》入口指向图像分类章节。

### 2026 年 05 月

博客写作的重构原则：

- 写应用时：把自己当成产品经理 + 全栈工程师。关注流程、库的使用、效果展示。
- 写原理时：把自己当成研究生 + 数学家。关注公式、结构图、论文复现。

1. 模块化了深度学习的内容，将《激活函数》、《损失函数》、《优化器》解构出来，仅作为知识的记录，在《网络结构》与《跨时代算法》中，将这些模块的作用和意义进行了整理和说明。
2. 创建并更新了《激活函数》中的 “Sigmoid 激活函数” 与 “ReLU 激活函数” 章节。
3. 创建并更新了《损失函数》中的 “MSE：均方误差” 与 “CELoss：交叉熵损失” 章节。
4. 创建并更新了《优化器》中的 “SGD”、“Adam”、“AdamW”、“Adafactor” 章节。
5. 创建并更新了《网络结构》中的 “MLP”、“CNN”、“AlexNet”、“VGG”、“ResNet” 章节。
6. 更新了《激活函数》中的 “Tanh”、“SiLU”、“HardSwish”、“SoftMax” 章节。
...
- 更新了《图像处理》中 “锐化与均衡化”、“形态学与运算”、“视频流处理”、“阈值分割”、“边缘与轮廓检测” 、“绘制图形与文本” 章节。
